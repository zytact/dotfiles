...
/** Map the shared scale to Claude's native adaptive-thinking controls. */
function claudeReasoningOptions(effort: ReasoningEffort | undefined): {
  ...
  if (effort) return { thinking: { type: "adaptive" }, effort };
  return {};
}
...
const makeClaudeSession = (
  ...
  Effect.gen(function* () {
    ...
    const reasoningOptions = claudeReasoningOptions(task.reasoningEffort);
    if (task.reasoningEffort === "off") {
      state.meta = { ...state.meta, reasoningEffort: "off" };
    }
    const captureEffectiveEffort: HookCallback = async (input) => {
      const effective = input.effort?.level;
      if (effective && effective !== state.meta.reasoningEffort) {
        state.meta = { ...state.meta, reasoningEffort: effective };
        emit({
          _tag: "MetaChanged",
          meta: { reasoningEffort: effective },
        });
      }
      return {};
    };
      ...
      try: () =>
        query({
          ...
          options: {
            ...
            ...reasoningOptions,
            // Hook input reports the effective per-turn effort after any
            // model-specific silent downgrade.
            hooks: {
              PreToolUse: [{ hooks: [captureEffectiveEffort] }],
              Stop: [{ hooks: [captureEffectiveEffort] }],
            },
          },
        }),
